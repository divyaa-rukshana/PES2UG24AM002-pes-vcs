...
static int write_tree_recursive(IndexEntry *entries, int count, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        if (!slash) {
            ...
            ObjectID sub_id;
            write_tree_recursive(sub_entries, sub_count, &sub_id);
    ...
    if (tree_serialize(&tree, &buffer, &size) != 0) return -1;
...
int tree_from_index(ObjectID *id_out) {
    ...
    for (int i = 0; i < index.count - 1; i++) {
        for (int j = i + 1; j < index.count; j++) {
            if (strcmp(index.entries[i].path,
                       index.entries[j].path) > 0) {

                IndexEntry temp = index.entries[i];
